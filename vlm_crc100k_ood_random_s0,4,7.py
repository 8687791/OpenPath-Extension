...
def seed_torch(seed=3407):  # ⚡️ 默认随机种子同步固化为 3407
    random.seed(seed)
...
def get_arguments():
    ...
    parser.add_argument("--seed", default=3407, type=int)  # ⚡️ 同步对齐学术种子 3407
    parser.add_argument("--model_type", type=str, default='combine')
    
    # 🎯 核心修改一：将 1.湿疹(0), 5.色素痣(4), 8.脂溢性角化病(7) 设为真实已知类 (ID)
    parser.add_argument("--id_cls", nargs="+", type=int, default=[0, 4, 7])
    # 🎯 核心修改二：其余 7 类恶性肿瘤与杂病（如 Melanoma, BCC）自动退归为 OOD 干扰拦截池
    parser.add_argument("--ood_cls", nargs="+", type=int, default=[1, 2, 3, 5, 6, 8, 9])
    
    # ⚡️ 修改保存地址至 clip_query_round_2.csv，防止历史版本被覆盖
    parser.add_argument("--save_csv", type=str, default="al_file_skin/clip_query_round_old_047.csv")
...
def kmean_cluster(embeds, n):
    cluster_learner = KMeans(n_clusters=n, init='k-means++', n_init='auto', random_state=3407)  # ⚡️ 对齐种子
...
def zero_shot_inference_random(args, train_eval_loader, id_cls, model_type='combine'):
    ...
    # 标准 10 分类临床医学提示词矩阵
    skin_text_prompts = [
        "A dermoscopy image of Eczema showing erythematous plaques",                    # 0. 湿疹 (ID 目标)
        "A dermoscopy image of malignant Melanoma with irregular pigmentation",          # 1. 黑色素瘤 
        ...
        "A dermoscopy image of Seborrheic Keratoses with stuck-on appearance",           # 7. 脂溢性角化病 (ID 目标)
    ...
    # 构建科学的 OOD 外部混淆文本干扰库（杂病噪音），用于迷惑 CLIP 分类边界，提取高质边界特征
    ...
    # 🎯 核心修改三：动态抽取出选定的 [0, 4, 7] 三个目标 ID 提示词，并无缝结合前 3 项随机干扰描述
...
if __name__ == "__main__":
    ...
    # 🎯 核心修改四：由于 id_cls 长度为 3，硬分类索引 <= 2 统统判定为 VLM 选中的已知常见病
