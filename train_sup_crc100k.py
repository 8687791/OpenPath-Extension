...
def get_arguments():
    ...
    parser.add_argument("--query_times", type=int, default=5)#次数5
...
def cal_acc(y_pred, y_true):
    ...
    return test_accuracy, f1, r, p  # 如果需要返回值
...
if __name__ == "__main__":
    ...
    import os
    # 提取初始化 CSV 中所有图片的文件名（不带路径，例如 MUS-RKAVKGDD.tif）
    ...
    for i in range(query_times):
        ...
        if i == 0:
            ...
            model = BMC_Vision_FT_Lit.load_from_checkpoint(checkpoint_path=ckpt_path, pretrain=True,
                                                           num_class=len(id_cls), args=args)
            save_csv = f'al_file/BMC_query{i}_labeled.csv'
            cur_precision = sample_selection(args, model, labeled_loader, selection_loader, query_num=args.query_num,
                                             save_dir=save_csv)
            save_dir = f'lightning_logs/CRC100K-BMC-ST-{i}-L/'
            labeled_data, _ = get_files(save_csv)
            labeled_data_all += labeled_data
