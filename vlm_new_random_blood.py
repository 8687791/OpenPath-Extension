...
# 💡 导入 Tumor_dataset_val_cls（纯分类 Dataset，不加载 Mask）
from dataset.alb_dataset2 import Tumor_dataset_val_cls, get_loader

# 💡 全局类别映射表，严格对应 0~3 索引 (血细胞数据集)
CLASS_MAPPING = {
    0: {"abbr": "EOSINOPHIL", "tag": "eosinophil"},
    1: {"abbr": "LYMPHOCYTE", "tag": "lymphocyte"},
    2: {"abbr": "MONOCYTE",   "tag": "monocyte"},
    3: {"abbr": "NEUTROPHIL", "tag": "neutrophil"}
}
...
def generative_ood_discovery(args, dataloader, model, tokenizer, truth_dict, id_cls):
    ...
    id_predicted_tags = []
    ood_descriptions = []
    debug_reports = [] # ✨ 全局抓取机制
    
    running_true_id_in_pool = 0 
    
    # ✨ 强化版 Prompt
    vqa_prompt = (
        "You are an expert hematopathologist. Focus on the PRIMARY White Blood Cell (WBC) in this image. "
        "Even if there are red blood cells, platelets, or background artifacts, you MUST classify the primary WBC into EXACTLY ONE of the following 4 categories:\n"
        "1. EOSINOPHIL\n"
        "2. LYMPHOCYTE\n"
        "3. MONOCYTE\n"
        "4. NEUTROPHIL\n\n"
        "Respond STRICTLY in this format (DO NOT use markdown asterisks like **):\n"
        "Category: [Input exactly one category abbreviation]\n"
    ...
    print("\n🔍 开始基于 4-Way 临床单选的生成式阅片与 OOD 精准拦截...")
    ...
    for idx, sample in enumerate(pbar):
        ...
        with torch.no_grad():
            ...
            ood_tags = [CLASS_MAPPING[idx]['tag'] for idx in range(4) if idx not in id_cls]
            ...
            if not matched_tag:
                for tag in gold_id_tags:
                    if tag in report_lower:
                        if not any(o_tag in report_lower for o_tag in ood_tags):
                            matched_tag = tag
                            break
            ...
            if matched_tag is not None:
                ...
                label_val = truth_dict.get(img_name, truth_dict.get(base_name))
            ...
            debug_reports.append({
                ...
                "matched_tag": matched_tag if matched_tag else "None",
                "raw_output": generated_report
            })
    ...
    # ✨ 导出全量 Debug 报告
    debug_file_path = os.path.join(args.output_dir, "debug_all_reports.json")
    with open(debug_file_path, "w", encoding='utf-8') as f:
        json.dump(debug_reports, f, indent=4, ensure_ascii=False)
    print(f"🐛 Debug 文件已生成，请查看大模型原始输出: {debug_file_path}")
    ...
    if len(ood_descriptions) > 0:
        ...
        with open(os.path.join(args.output_dir, "discovered_ood_clusters.json"), "w", encoding='utf-8') as f:
...
def main():
    ...
    parser.add_argument("--init_num", default=45, type=int)
    ...
    id_cls = sorted(np.random.choice(all_classes, size=2, replace=False).tolist())
    ood_cls = sorted([c for c in all_classes if c not in id_cls])
    
    print("="*50)
    print(f"🎲 随机种子 (Seed): {args.seed}")
    print(f"🟢 随机抽取的 2 个已知类 (ID): {id_cls}")
    print(f"🔴 剩下的 2 个未知类 (OOD): {ood_cls}")
    print("="*50)

    csv_path = '/root/gpufree-data/OpenPath-main/data/blood_cells_data/train.csv'
    if not os.path.exists(csv_path):
        if os.path.exists('al_file/train.csv'):
            csv_path = 'al_file/train.csv'
        else:
            print(f"❌ 错误：找不到数据集 CSV 文件: {csv_path}")
            return

    train_files, truth_dict = get_files(csv_path)
    
    if len(train_files) > 200:
        print(f"⚠️ 检测到数据总量为 {len(train_files)} 张。正在随机采样 200 张作为冷启动精选池...")
        random.seed(args.seed)
        train_files = random.sample(train_files, 200)

    train_dataset = Tumor_dataset_val_cls(args, files=train_files)
    ...
    vlm_raw_id_count = sum([1 for n in names_id_vlm if truth_dict.get(n, truth_dict.get(os.path.basename(n))) in id_cls or str(truth_dict.get(n, truth_dict.get(os.path.basename(n)))) in [str(c) for c in id_cls]])
    ...
    labels_b = [truth_dict.get(n, truth_dict.get(os.path.basename(n), 'Unknown')) for n in selected_b]
    pd.DataFrame({'img': selected_b, 'label': labels_b}).to_csv(csv_b, index=False)
    
    labels_c = [truth_dict.get(n, truth_dict.get(os.path.basename(n), 'Unknown')) for n in selected_c]
    pd.DataFrame({'img': selected_c, 'label': labels_c}).to_csv(csv_c, index=False)

    def calc_qp_report(selected_list):
        id_count = sum([1 for n in selected_list if truth_dict.get(n, truth_dict.get(os.path.basename(n))) in id_cls or str(truth_dict.get(n, truth_dict.get(os.path.basename(n)))) in [str(c) for c in id_cls]])
