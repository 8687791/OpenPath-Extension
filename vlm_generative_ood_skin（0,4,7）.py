...
def load_internvl(model_path="pretrained/InternVL2_5-8B"):
    print(f"🚀 正在加载旗舰级 InternVL2.5-8B 基础大模型...")
...
def generative_ood_discovery(args, dataloader, model, tokenizer):
    ...
    # 🎯 文本词根同步更新：让大模型圈出湿疹、色素痣以及脂溢性角化病
    gold_id_tags = ['eczema', 'melanocytic nevi', 'nevi', 'seborrheic keratoses']
...
def main():
    ...
    # ⚡️ 使用全新的 3407 固化种子进行冷启动精选池空间的随机采样
    if len(train_df) > 500:
    ...
    # 🎯 核心变更：真正物理目标类转换为 0=湿疹, 4=色素痣, 7=脂溢性角化病
    gold_id_classes = [0, 4, 7] 
    ...
    print(f" ├─ 真正目标类 (ID: 0=Ecz, 4=Nevi, 7=SK) 数量: {vlm_raw_id_count}")
    ...
    # 结果双轨落盘
    csv_b = os.path.join(args.output_dir, "query_round_047_density.csv")
    csv_c = os.path.join(args.output_dir, "query_round_047_graph.csv")
