...
def main():
    ...
    # 修改点：默认保存在指定的固定文件夹中
    parser.add_argument("--output_dir", default="/root/gpufree-data/OpenPath-main/al_file", type=str)
    ...
    # 修改点：将两个生成的 csv 文件保存至你要求的统一路径下
    csv_b = os.path.join(args.output_dir, "query_round_5_density7.csv")
    csv_c = os.path.join(args.output_dir, "query_round_5_graph7.csv")
...
🔬 [方案 B: Density-Core FPS] (已保存至: {csv_b})
 ├─ 真正目标类(ID): {b_id} | 杂病(OOD): {b_ood}
 └─ 💡 最终精准度(QP): {b_qp:.2f}%

🔬 [方案 C: Graph Centrality NMS] (已保存至: {csv_c})
