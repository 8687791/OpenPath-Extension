...
def main():
    ...
    if not os.path.exists(args.train_csv):
        print(f"❌ 错误：找不到 {args.train_csv}，请确保已经提前运行过 python make_csv_skin.py")
        return

    train_df = pd.read_csv(args.train_csv)
    
    # 💡 核心修改点 5：因为你洗出来的 train.csv 有整整 20040 张切片，全量扫网大模型会非常慢。
    # 这里我们精准随机抽取 1000 张图送入大模型，进行冷启动盲测初筛，既保证了统计多样性，又做到了光速通关。
    if len(train_df) > 1000:
        print(f"⚠️ 检测到皮肤原始数据量庞大 ({len(train_df)}张)。正在随机采样 1000 张作为冷启动精选池...")
    ...
    out_csv = os.path.join(args.output_dir, "query_round_4.csv")
    df_selected.to_csv(out_csv, index=False)
    print(f"✅ 皮肤数据集生成式冷启动挑选完成！结果已成功隔离保存至 {out_csv}")
